...
resource "aws_iam_role" "apprunner_ecr_kore_miteyo" {
...
resource "aws_iam_role_policy" "apprunner_ecr_kore_miteyo" {
  name   = "apprunner-ecr-access-policy"
  role   = aws_iam_role.apprunner_ecr_kore_miteyo.name
...
resource "aws_iam_role" "codebuild_kore_miteyo" {
  name               = "codebuild-kore-miteyo-assume"
  assume_role_policy = data.aws_iam_policy_document.codebuild_assumerole.json
}

data "aws_iam_policy_document" "codebuild_kore_miteyo" {
  statement {
    effect = "Allow"
  ...
  statement {
    ...
    condition {
      ...
      values = var.kore_miteyo_private_subnet_arns
    }
...
resource "aws_iam_role_policy" "kore_miteyo" {
  role   = aws_iam_role.codebuild_kore_miteyo.name
  policy = data.aws_iam_policy_document.codebuild_kore_miteyo.json
}
